...
export function fetchBook(_id) {
  return dispatch => {
    axios
      .get(BASE_URL + "/books/viewbook/" + _id)
      .then(function(response) {
        //console.log("API RESPONSE" + response);
        dispatch({
          type: "FETCH_BOOK",
...
export function updateBook(book, _id) {
  return dispatch => {
    axios
      .post(BASE_URL + "/books/update/" + _id, book)
      .then(function(response) {
        //console.log("API RESPONSE" + response);
        dispatch({
          type: "UPDATE_BOOK",
...
export function deleteBook(_id) {
  return dispatch => {
    axios.get(BASE_URL + "/books/delete/" + _id).then(function(response) {
      dispatch({
        type: "DELETE_BOOK",
        payload: response
      });
